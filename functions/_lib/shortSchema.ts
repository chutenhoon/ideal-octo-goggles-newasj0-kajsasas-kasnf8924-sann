...
export async function ensureShortsSchema(env: Env) {
  const info = await env.DB.prepare("PRAGMA table_info(shorts)").all<ColumnRow>();
  ...
  for (const column of ADD_COLUMNS) {
    if (!existing.has(column.name)) {
      await env.DB.prepare(
        `ALTER TABLE shorts ADD COLUMN ${column.name} ${column.type}`
      ).run();
    }
  }
  ...
  if (!existing.has("pc_key")) {
    await env.DB.prepare(
      "UPDATE shorts SET pc_key = r2_key WHERE pc_key IS NULL AND r2_key IS NOT NULL"
    ).run();
  }

  if (!existing.has("thumb_key")) {
    await env.DB.prepare(
      "UPDATE shorts SET thumb_key = thumbnail_key WHERE thumb_key IS NULL AND thumbnail_key IS NOT NULL"
    ).run();
  }
}
